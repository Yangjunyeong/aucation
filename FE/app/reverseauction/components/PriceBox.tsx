  ...
  return (
    <div className="rounded-lg flex flex-row items-center p-6 bg-gray-100 border border-gray-400 mt-14">
      <div className="flex flex-col items-start flex-1 ml-4">
        ...
        <p className="text-xl font-semibold text-customLightTextColor">{pricetoString(startingPrice)}</p>
      </div>
      ...
      {endPrice == 0 && (
        <div className="flex flex-col items-start flex-1 ml-10">
          <h3 className="text-sm text-customGray">최저가</h3>
          <p className="text-xl font-semibold text-customBlue">{pricetoString(lowPrice)}</p>
        </div>
      )}
    </div>
  );
